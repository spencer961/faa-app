          ...
          {[...shown].sort((a, b) => (getPractices(a).length > 1 ? 1 : 0) - (getPractices(b).length > 1 ? 1 : 0)).map((c) => {
            ...
            return (
              <div key={c.id} onClick={() => setDetailId(c.id)} style={{ ...card, gridColumn: multi ? '1 / -1' : undefined }}>
                ...
                {/* Fixed-height pill row (to-dos + to-review) so cards line up whether or not pills are present. */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', minHeight: 24, marginBottom: 10 }}>
                  {ct.todos && openN > 0 && (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 5, flex: 1 }}>
                  ...
                  <div style={cvis.length > 7 ? { display: 'flex', flexDirection: 'column', gap: 5, maxHeight: 236, overflowY: 'auto' } : { display: 'contents' }}>
                    {cvis.map((l) => <LinkChip key={l.id} l={l} clientName={c.name} editMode={editMode} onEdit={(e) => { e.stopPropagation(); setLinkModal({ id: l.id, clientId: c.id }) }} onDelete={(e) => { e.stopPropagation(); if (window.confirm('Delete the “' + l.label + '” link?')) deleteLink(l.id) }} />)}
                  </div>
...
function Detail({ client: c, links, onBack, clients, onSaveLink, onDeleteLink, onSavePractices, tiers, onToggleTier, onSetCadence, onSaveAbbr, onSetColor, onSaveClient, onDeleteClient, onArchive, onSaveNotes, onSavePayment, onDeletePayment, onSaveBilling, onEditPayment, canUndo, onUndo, toast, clientMode, clientModeName, onExitClientMode, cmSince, hiddenSections }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG }}>
      <Header sub="Client Detail" back={<button onClick={onBack} style={HBTN}>{clientMode ? '← Back' : '← All clients'}</button>} right={clientMode ? (
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: '#fff', borderRadius: 14, padding: 24, width: 460, maxWidth: '95vw' }}>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16, borderTop: '0.5px solid rgba(0,0,0,0.08)', paddingTop: 14 }}>
          {link && <button onClick={() => { if (window.confirm('Delete this link? This can’t be undone.')) onDelete(link.id) }} style={{ ...btnGhost, color: '#A32D2D', borderColor: 'rgba(163,45,45,0.3)', marginRight: 'auto' }}>Delete</button>}
